...
module.exports = class BaseGenerator extends Generator {
  ...
  checkPackage() {
    if (_.isEmpty(this.pkg)) {
      this.log.error(
        'Could not find a valid package.json. Did you generate a new application and are running the generator in the project directory?'
      );
    ...
    if (!(this.pkg.directories && this.pkg.directories.lib)) {
      this.log.error(
        'It does not look like this application has been generated with this version of the generator or the required `directories.lib` has been removed from package.json.'
      );
      return process.exit(1);
    }
  }
  ...
  get hasAsync() {
    const asyncNode = '>= 8.9.0';
    const pkgEngineAsync =
      !(this.pkg.engines && this.pkg.engines.node) ||
